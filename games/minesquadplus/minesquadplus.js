...
function setup() {
  const canvas = createCanvas(
    boardWidth * squareHeight,
    boardHeight * squareHeight + footerHeight
  );
  canvas.parent("game");
  initGame();
}
...
function draw() {
  ...
  noFill();
  rect(
    Math.floor(mouseX / squareHeight) * squareHeight + 1,
    Math.floor(mouseY / squareHeight) * squareHeight + 1,
    squareHeight - 2,
    squareHeight - 2
  );
  ...
  for (let i = 0; i < squadsLeft; i++) {
    ...
    ellipse(
      squareHeight / 2 + squareHeight * 2 + i * (squareHeight * 2),
      boardHeight * squareHeight + minesLeftBox - squareHeight / 2,
      bombHeight,
      bombHeight
    );
  }
  ...
  text(
    "" + score,
    squareHeight / 2 + squareHeight * 2,
    boardHeight * squareHeight + minesLeftBox - squareHeight / 2 + squareHeight
  );
}
...
function getNeighbors(tile) {
  ...
  if (getNeighbor(tile, topLeft)) {
    neighbors.push(topLeft);
  }
  if (getNeighbor(tile, topCenter)) {
    neighbors.push(topCenter);
  }
  if (getNeighbor(tile, topRight)) {
    neighbors.push(topRight);
  }
  if (getNeighbor(tile, midLeft)) {
    neighbors.push(midLeft);
  }
  if (getNeighbor(tile, midRight)) {
    neighbors.push(midRight);
  }
  if (getNeighbor(tile, btmLeft)) {
    neighbors.push(btmLeft);
  }
  if (getNeighbor(tile, btmCenter)) {
    neighbors.push(btmCenter);
  }
  if (getNeighbor(tile, btmRight)) {
